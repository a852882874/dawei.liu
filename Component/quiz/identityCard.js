...
export default class IdentityCardClass extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            avatarSource2: "",
            avatarSource2Base64: "",
            user_sex: this.props.sex ? this.props.sex : "female",
            visible: false,
            visiblenum: 0,
            id: "",
            Loadding: false,
    ...
    render() {
        return (
            <View style={IdentityCard.wrap}>
                <Header title={"身份认证"} type={"noback"} {... this.props} sex={ this.state.sex } />
                {this.Loadding()}
                ...
                _ajax.post_image_token("user/image/upload", formData, function (res) {
                    ...
                    if (res.code == 0) {
                        if (that.state.username != "") {
                            _ajax.get_token("user/self_manage", that.props.navigator, function (ress) {
                                if (ress.code == 0) {
                                    AsyncStorage.setItem('UserList', JSON.stringify({ UserList: ress.user }), () => {
                                        that.props.navigator.replace({
                                            component: that.state.user_sex == "male" ? LUyou : SearchAndm,
                                            reset: true,
                                            params: {
