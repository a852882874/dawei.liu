...
export default class extends Component {
  ...
  renderPagination = () => {
    ...
    const ActiveDot = this.props.activeDot || <View>
      <Text style={{ color: "#828282", fontSize: 13 }}>{this.state.index + 1}/15</Text>
    </View>
    const Dot = this.props.dot || <View>
      <Text style={{ color: "#b1b1b1", fontSize: 13 }}>{this.state.index + 1}/15</Text>
    </View>
    for (let i = 0; i < this.state.total; i++) {
      dots = (i === this.state.index
    ...
    return (
      <View pointerEvents='none' style={[styles['pagination_' + this.state.dir], this.props.paginationStyle, { position: "absolute", top: -490 }]} >
        {dots}
      </View>
    )
  }
  ...
  componentWillReceiveProps(nextProps) {
    let button = null
    if (nextProps.aaa == true) {
      if (nextProps.loop ||
        this.state.index !== this.state.total - 1) {
        button = "wqeqweqweq"
      }
      nextProps.bbb(this.state.index + 1)
  ...
  renderPrevButton = () => {
    ...
    if (this.props.loop || this.state.index !== 0) {
      ...
      button = this.props.prevButtonNo || <Text style={styles.buttonText}>‹</Text>
    }
